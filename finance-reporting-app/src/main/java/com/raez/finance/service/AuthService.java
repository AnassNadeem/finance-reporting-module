...
public class AuthService {
    ...
    public void requestPasswordResetEmail(String accountLookup, String sendToEmail) {
        ...
        try {
            FUser user = fUserDao.findByEmailOrUsername(accountLookup.trim());
            ...
            if (!ValidationUtils.isValidEmailFormat(to))
            ...
            if (gs.isSmtpEnabled() && gs.getSmtpHost() != null && !gs.getSmtpHost().isBlank()) {
                try {
                    MailService.sendPasswordResetEmail(to, token, user.getUsername());
                } catch (Exception ex) {
