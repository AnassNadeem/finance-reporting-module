...
public final class DBConnection {
    ...
    /** After a successful run of {@link DatabaseInitialiser#applyCompatibilityMigrations}. */
    private static boolean compatMigrationsDone = false;
    ...
    public static Connection getConnection() throws SQLException {
        ...
        if (!bootstrapping && !compatMigrationsDone) {
            try {
                DatabaseInitialiser.applyCompatibilityMigrations(conn);
                compatMigrationsDone = true;
            } catch (SQLException ex) {
                System.err.println("[DB] Compatibility migrations failed: " + ex.getMessage());
                ex.printStackTrace();
            }
        }
